...
public class FallingStarsEffect extends AbstractGameEffect {
    ...
    public FallingStarsEffect(int frostCount, boolean flipped) {
        ...
        this.color = new Color(MathUtils.random(0.7F, 1.0F), 0.1F, MathUtils.random(0.7F, 1.0F), MathUtils.random(0.9F, 1.0F));
    ...
    public void update() {
        ...
        if (!(this.waitTimer > 0.0F)) {
            ...
            if (this.y < this.floorY) {
                ...
                CardCrawlGame.sound.playA("ATTACK_MAGIC_FAST_1", pitch);

                for(int i = 0; i < 4; ++i) {
                    AbstractDungeon.effectsQueue.add(new StarShatterEffect(this.x, this.y));
                }
